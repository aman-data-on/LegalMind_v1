...
/**
 * Review list and one Review's detail — **the detail view is the load-bearing
 * surface** (locked 52.5, 52.6 Steps 9, 30, 49.6).
 *
 * Both views live at the fixed pathname `/reviews`; which one renders is
 ...
 * Everything below the list follows from that.
 *
 * The five properties the detail view exists to guarantee:
 *
...
import { SkeletonFindings, SkeletonTable } from "@/components/Skeleton";
              ...
              <tbody>
                {reviews.map((review) => (
                  <tr key={review.id}>
                    ...
                    <td>
                      <Link href={`/contracts?id=${review.contract_id}`}>
      ...
      {review ? (
        <p className="page-meta">
          ...
          */}
          ...
          <Link href={`/reviews/report?id=${reviewId}`}>View report</Link>
        </p>
      ) : null}
...
function ReviewsPageInner() {
  ...
  return reviewId ? (
    <ReviewDetailView key={reviewId} reviewId={reviewId} />
  ) : (
    <ReviewsListView />
  );
}
...
export default function ReviewsPage() {
  return (
    <Suspense fallback={<Loading what="reviews" />}>
      <ReviewsPageInner />
    </Suspense>
  );
}
