...
boundaries — the one file both PR #69 (planner) and PR #74 (reranker) rewrite — and a
new auth-bearing endpoint cannot be verified while no test database is provisioned.
Suggested follow-up chips have no deterministic source until the planner ships.
...
**Verification**, measured after rebasing onto the workspace fix pass (PR #23) rather than
against the pre-#23 tree: backend 1454 → **1488 passed**, 142 skipped (PR #23's own count,
unchanged by this work), 1 xfailed; frontend 351 → **380 passed** in 28 files, typecheck and
`check:terms` clean, production build clean. So Client Profiles adds **+34 backend and +29
frontend** tests and hides no skip. **12 new browser tests** (`e2e/client-profiles.spec.ts`)
