...
**3. Decision IDs are the fastest way in.** Every locked decision has a stable ID; search the registry by prefix rather than reading `all_lock.md` end to end: `PROD` `ROLE` `LEGAL` `FIND` `DOC` `ENG` `AI` `ARCH` `DATA` `AUD` `LIABILITY` (Steps 1–45B) · `REC` (reconciliation) · `AM` (Amendment Batches AB-1 `AM-1`–`AM-21`, AB-2 `AM-22`–`AM-24`, and AB-3 onward through `AM-54`) · `SEC` (Step 47) · `API` `FE` `OBS` `TEST` `DEP` (Steps 49, 52–55) · `IMPL` (implementation authorization). Open items use `OD-*` (open decisions), `C-*` (conflicts), `F-*` (engineering resolutions).
