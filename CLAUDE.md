...
**Company Standards are PER DOCUMENT TYPE (owner Q3=B, 2026-08-19). There is no Legal Rule at all.**
...
* **Wiring status:** the label is not yet configured in the engine — the threshold keys cannot express zero tolerance (`acceptable_max = preferred` would wrongly ACCEPT below-preferred values), so it needs a small engine addition (a `deviation_outcome` config key). **Routing is already correct either way**: locked D-3.5(a) sends `UNACCEPTABLE` to a human and `UNRULED_DEVIATION_REQUIRES_DECISION` sends unlabelled deviations to the same place. Until wired, a deviation carrying `NOT_APPLICABLE` is correct and still reaches Legal.
