...
**What already exists:** the frontend (`frontend/`) is not a blank slate. All ten Step 52.6 screens except report export are implemented, permission-gated, and covered by 58 Vitest tests and a Playwright browser suite that pins down real security- and correctness-critical DOM behavior (confidentiality omission, no-optimistic-decision-UI, 409 conflict surfacing, byte-identical 404s). Until 2026-09-10, deliberately, no CSS framework, component library, or client-state library was added — `frontend/src/app/globals.css` was one plain stylesheet. **Owner, 2026-09-10: shadcn/ui + Tailwind is now approved, incrementally, as the component foundation** — see [CLAUDE.md](CLAUDE.md) § UI and UX work for the binding terms (no big-bang migration, existing visual language wins, the five-state-axis and confidential-omission surfaces are never touched). This document governs how that gets filled in, not whether the existing information architecture is correct. **The IA is correct and locked-adjacent (52.5); the visual and component layer is what's open.**
