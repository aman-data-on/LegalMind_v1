...
- [00-project/ASK_TARGET_ARCHITECTURE.md](00-project/ASK_TARGET_ARCHITECTURE.md) — the Ask target architecture the owner approved 2026-09-16, whose **§0 is the canonical ten-stage map** (conversation context → query understanding → knowledge router + authority policy → query planner → authorized targeted retrieval → reranker → evidence sufficiency gate → grounded generation → answer verification → response), recorded 2026-09-17 with the shipped state of each stage: current vs target, request flow with the five worked examples, division of labour between Gemini, RAG and the deterministic systems, which locked decisions stay and which would need amendment, the phased migration and how "smarter" is measured. Implemented phase by phase; decides nothing
- [00-project/QUERY_UNDERSTANDING_DESIGN.md](00-project/QUERY_UNDERSTANDING_DESIGN.md) — 📁 ANALYSIS, 2026-09-23. Why Ask cannot represent a question: understanding is 17 call sites across five modules, no object holds it, and `RoutePlan.domains` is set membership rather than a plan. Scores the eleven understanding dimensions (two explicit, four regex, two inferred, three missing), shows why the penalty/deadline/multi-domain failures are three different diseases that no extra signal can fix, proposes the `QuestionUnderstanding` contract with `requested_fact`, and sets out the four-step migration. Decides nothing
...
- [00-project/CAPABILITY_MANIFEST_PROPOSAL.md](00-project/CAPABILITY_MANIFEST_PROPOSAL.md) — 📁 **PROPOSAL** — the capability manifest `AM-68` r3 would make the only evidence for a capability answer: every entry a built-and-tested feature with its endpoint and test named, plus five explicit limits. **Not approved, nothing reads it**
- [00-project/AB21_PROPOSED_AMENDMENTS.md](00-project/AB21_PROPOSED_AMENDMENTS.md) — 📁 **PROPOSAL** — drafts of `AM-67` (Domain A as a controlled reading aid), `AM-68` (the capability question shape), `AM-69` (language is not a safety boundary) and `AM-70` (multilingual embedding, deferred). **Not approved, not in `all_lock.md`**
...
| [TAILWIND_ISOLATION.md](design/TAILWIND_ISOLATION.md) | 📁 How Tailwind was proven not to touch any existing screen — the method, the hashes, and the one thing it does change |
