...
- [00-project/ASK_TARGET_ARCHITECTURE.md](00-project/ASK_TARGET_ARCHITECTURE.md) — the Ask target architecture the owner approved 2026-09-16 (understand → plan → retrieve → rerank → verify → answer): current vs target, request flow with the five worked examples, division of labour between Gemini, RAG and the deterministic systems, which locked decisions stay and which would need amendment, the phased migration and how "smarter" is measured. Implemented phase by phase; decides nothing
