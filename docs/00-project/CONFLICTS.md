...
| C-23 | Legal Constitution L1.10 §31 defines company positions for **Partner Agreement, Vendor Agreement, Distribution Agreement and Purchase Order**, and tags clauses with those names under "Applicable Document Types" — but locked **Step 6's ten Document Types carry none of them**, and no ratified or proposed Company Standard covers any of the four | ⏳ Open (MEDIUM) — needs an owner decision on whether Step 6's vocabulary gains the §31 types and whether those positions are ratified into standards. Ask now REFUSES a question naming one instead of answering it with an MSA position (2026-09-18); later the same day the two general mechanisms that produced the wrong answer were fixed in Domain A retrieval itself (provenance no longer indexed; the §31 family's bare subject "partner" is filtered by name — CHANGELOG; a shut-gate verdict was tried and reverted because the document-calibrated floor refuses real Domain A paraphrases at 0.45–0.47), so the refusal also covers phrasings that name no paper ("what does our constitution say about partners"). The refusal is correct under either reading, so this blocks no code — but it is the ceiling: the reader cannot be told what §31.3–31.6 say until they are ratified into standards, which needs this decision |
