...
**`REC-08` was added later, on 2026-08-17**, and is therefore *not* part of the 2026-08-16 session described above. It is recorded in [`all_lock.md`](../../all_lock.md) under **"Reconciliation Decision REC-08 — CI/CD tooling"**, appended after Amendment Batch AB-2 (the prior 15,093 lines are byte-identical and unmodified). Like `REC-01`–`REC-07` it resolves how existing locked decisions relate to each other; unlike them it also supersedes one line item of a locked list, which is why it carries its own lock record.
