...
Backend: **937 tests**, 1 skipped (slice 4 added the frontend-vocabulary parity guard) — the skip is `test_assist_schema.py`'s `legalmind_assist` grant check, which asserts its property only where the role exists; the role is a deployment precondition and `preflight` reports its absence as BLOCKED — the six supplied documents are present at `LEGALMIND_SOURCE_MATERIAL_DIR` and parse `COMPLETE`. Frontend: **102 Vitest + 50 Playwright** (54 runner items with setup; 18 further specs gated — 15 visual baselines, 3 documentation captures). All green. `ruff` and `mypy` are at **zero** and CI job 1 is blocking. CI is **15 jobs** (job 14 added 2026-08-26 — dependency and container scanning, A10; job 15 added 2026-08-27 — Design QA: the rule-12 forbidden-terms gate and 0.1%-threshold visual regression) and, since 2026-08-26, **runs on every branch push** — under the previous `main`-only trigger no job had run for five days of feature-branch commits.
