...
| 11 · Observability & deployment | Steps 53, 55 | IMPLEMENTED · TESTED | **Step 53 complete** — 53.1–53.5 implemented, all seven named signals now have emission sites (36 tests). **Step 55 complete as specifiable** — **22-check** preflight register covering every 55.2 row (the "18-check" figure predated the four assist-lane checks; count verified by running it, 2026-08-26), the 55.4 r3 / 55.5 post-migration reproducibility gate (`tools/verify_reproducibility`), queue-backed worker, Dockerfiles, compose — now network-segmented: `data` internal (no route out) vs `edge`, with model weights mounted read-only (25 tests). CI: **14 jobs** (see unit 12 — job 14, A10, added 2026-08-26), job 1 blocking at a zero baseline. **Retention policy, export formats, OIDC, log aggregation, alert thresholds and the monitoring stack remain NOT YET SPECIFIED** and are reported as `BLOCKED`/`ATTEST` rather than assumed |
...
Backend: **901 tests**, 1 skipped — the skip is `test_assist_schema.py`'s `legalmind_assist` grant check, which asserts its property only where the role exists; the role is a deployment precondition and `preflight` reports its absence as BLOCKED — the six supplied documents are present at `LEGALMIND_SOURCE_MATERIAL_DIR` and parse `COMPLETE`. Frontend: **62 Vitest + 23 Playwright** (27 runner items with setup). All green. `ruff` and `mypy` are at **zero** and CI job 1 is blocking. CI is **14 jobs** (job 14 added 2026-08-26 — dependency and container scanning, A10) and, since 2026-08-26, **runs on every branch push** — under the previous `main`-only trigger no job had run for five days of feature-branch commits.
