...
| 8 · Frontend | Step 52 | IMPLEMENTED · TESTED | 10 of 10 locked 52.6 screens except export; **62** Vitest tests + **23** Playwright browser-workflow tests (27 runner items, including 4 setup steps). **2026-08-26: the core Review screen had crashed on load since 2026-08-24** (a hook declared below an early return — React #310), invisible to typecheck/build and to CI, which never ran on the working branch; fixed, the whole suite re-run green, CI now triggers on every branch push |
...
Backend: **935 tests**, 1 skipped — the skip is `test_assist_schema.py`'s `legalmind_assist` grant check, which asserts its property only where the role exists; the role is a deployment precondition and `preflight` reports its absence as BLOCKED — the six supplied documents are present at `LEGALMIND_SOURCE_MATERIAL_DIR` and parse `COMPLETE`. Frontend: **68 Vitest + 23 Playwright** (27 runner items with setup). All green. `ruff` and `mypy` are at **zero** and CI job 1 is blocking. CI is **14 jobs** (job 14 added 2026-08-26 — dependency and container scanning, A10) and, since 2026-08-26, **runs on every branch push** — under the previous `main`-only trigger no job had run for five days of feature-branch commits.
