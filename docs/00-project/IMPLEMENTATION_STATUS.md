...
| — | **`GET /auth/oidc/*` not implemented.** OIDC needs an approved JWT/JWKS dependency plus IdP configuration. `POST /reviews/{id}/export` left this row 2026-08-31: the owner's export directive specified the formats 49.12 had left open (**PDF, DOCX** — decisions #231/#232), and it is now IMPLEMENTED · TESTED (7 tests incl. the LEGAL-02 omission-in-the-file property; `export.generate` granted alongside `report.view`, flagged for ratification) | OIDC recorded in `api/permission_map.py` `NOT_IMPLEMENTED` |
