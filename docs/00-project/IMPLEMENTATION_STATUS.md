...
| 7 · HTTP API | Steps 43, 47, 49 | IMPLEMENTED · TESTED | **45 operations** in the frozen contract ([docs/api/openapi.json](../api/openapi.json), drift-tested): the 39 locked by 49.3 plus the assist-lane conversation API (create · list · get-with-citation-replay · ask) and a read projection of Evidence, all recorded as implementation additions in Step 49's record section (2026-08-26); 139 API tests + the contract-shape suite |
| 8 · Frontend | Step 52 | IMPLEMENTED · TESTED | 10 of 10 locked 52.6 screens except export; **79** Vitest tests + **26** Playwright browser-workflow tests (30 runner items, including 4 setup steps), plus **8 gated specs** (5 visual-regression baselines behind `DESIGN_QA=1`, 3 documentation captures behind `DOCS_SHOTS=1`) that stay out of the default run so it remains order-independent. **2026-08-26: the core Review screen had crashed on load since 2026-08-24** (a hook declared below an early return — React #310), invisible to typecheck/build and to CI, which never ran on the working branch; fixed, the whole suite re-run green, CI now triggers on every branch push. **2026-08-27 (Phase 4 hardening):** loading skeletons (layout-shape-matched, reduced-motion-safe), review-screen keyboard navigation (`n`/`p`/`d`/`?`; `a`/`r` prepare a decision and provably never POST one), and the 409 conflict flow now freezes the form until an explicit refresh (52.7) — full loop proven in the browser suite |
...
Backend: **935 tests**, 1 skipped — the skip is `test_assist_schema.py`'s `legalmind_assist` grant check, which asserts its property only where the role exists; the role is a deployment precondition and `preflight` reports its absence as BLOCKED — the six supplied documents are present at `LEGALMIND_SOURCE_MATERIAL_DIR` and parse `COMPLETE`. Frontend: **79 Vitest + 26 Playwright** (30 runner items with setup; 8 further specs gated — 5 visual baselines, 3 documentation captures). All green. `ruff` and `mypy` are at **zero** and CI job 1 is blocking. CI is **15 jobs** (job 14 added 2026-08-26 — dependency and container scanning, A10; job 15 added 2026-08-27 — Design QA: the rule-12 forbidden-terms gate and 0.1%-threshold visual regression) and, since 2026-08-26, **runs on every branch push** — under the previous `main`-only trigger no job had run for five days of feature-branch commits.
