...
| | |
|---|---|
| 🔒 **11 Sep — a real privacy hole closed, plus three quality fixes (branch `feat/p1-rag-quality`, NOT deployed)** | **The one that matters.** When you reopen an old Ask conversation, LegalMind used to re-show everything it had quoted — including the company's approved position — purely because the conversation was yours. It never re-checked whether you were *still* allowed to see it. So if someone's access to the approved positions was withdrawn, or a deal was handed to another colleague, the old conversation kept showing the text anyway. I proved it on a test account first, then fixed it: reopening now re-checks your permissions exactly as asking a fresh question does, and anything you may no longer see is simply absent rather than blanked out. Your own questions and answers stay readable; only the quoted source material is withheld. **Three quality items alongside it.** (1) A heading that points at a clause was borrowing that heading's score and pushing the clause that actually answered the question down the list — the P0 item I flagged. Fixed at the root: the clause is now scored on its own merits, and the measurement that had slipped went back up, with nothing else moving. (2) The plain-English explanation under each finding was being rejected about 6 times in 10. The cause was not the AI — the checker was comparing words too literally, so "Leapswitch's" never matched "Leapswitch" and "ends" never matched "end". I fixed the comparison rather than lowering the bar, which is unchanged. (3) I tested adding a faster search index and **rejected it on the numbers**: on your documents it would have lost 85% of the correct results, because it does not combine with filtering to one document. I built it in a throwaway transaction to measure, so your live database never had it. **Nothing is deployed and no production data was touched.** |
