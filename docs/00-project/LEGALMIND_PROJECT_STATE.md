...
| | |
|---|---|
| ✅ **23 Sep (later) — Ask now explains your standards in plain English and answers the situation you describe.** | **What you asked for.** When a customer says "we agreed 6 months of compensation", Ask should say whether that is our position — not just paste the clause. **What it says now.** "The approved position cited here does not state 6 months. The customer has no right to terminate … before the expiry of the Term. If the customer terminates for a reason not expressly permitted, they remain liable for an early termination fee equal to the total fees for the remainder of the Term. Any deviation from this position needs Legal review." **How it stays safe.** The check "does our standard say 6 months?" is done by exact code, not by the AI — I tested letting the AI word it and it let an invented condition through, so it does not. The AI still may not add any number, condition or exception that is not in the standard. **Also:** plain-English explanations were switched off on the live site; they are on now. One known gap: "end the MSA early" finds the liability cap first; "terminate early" finds the right standard. Recorded as `AM-78`. |
| ✅ **23 Sep — Ask no longer answers a legal question with an unrelated company standard. LIVE (`3114113`).** | **What was wrong.** Ask "what does Indian law say about penalty clauses?" and, when the law library had nothing, it showed you your governing-law and GST standards as "the organization's approved position relevant to this question" — only because they contain the words *Indian* and *law*. Twelve of the 76 test questions about the law did this, and with a contract open "who are the parties to this agreement?" showed a Partner Agreement notice rule. **What changed.** (1) A company standard is never offered as the answer to a question about the law — the law answers it, or Ask says the law library does not cover it. (2) When you did not ask about our standards, a standard is only shown if it is genuinely about your question, not merely sharing a word. (3) "Is this MSA ok for us?" now goes to the review findings, like "should we sign this?". **What did not change.** "liability cap??" with an MSA open, "what's our stance on auto renewal", termination, confidentiality, follow-ups and "quote it" all answer as before. Ask also makes fewer AI calls (1.87 → 1.24 per question). One trade-off: a two-word question like "governing law" with nothing open may now say not found — add "our" ("our governing law") and it answers. Recorded as `AM-77`. |
| ⛔ **21 Sep — the law library is fixed and Ask now knows a legal question when it sees one. FROZEN, and waiting on one thing from you: AI credit.** | **What was wrong, and is now fixed.** Two separate faults. First, the library itself: a tiny raised footnote number printed just before a section number came out glued to it, so the IT Act's section 66A read as "section 266A" — and because the software assumes section numbers only go up, one glued digit made it swallow **every section after it**. Six such lumps held **57% of all the law text**, and a lump matches almost any question, so **31% of everything the search returned was one**. Schedules (the tables at the back of an Act) had no address at all, so the DPDP Act's whole penalty table was being cited as "section 44, Amendments to certain Acts" — the wrong reference. Second, Ask could not tell a legal question from a contract question: it only recognised one when you NAMED the Act, which is **2 questions in 23**. The rest were thrown away by a safety check before they could be answered. **What changed.** The glued digit is stripped — but only where the Act's own contents page proves the number is impossible, and the repair **switches itself off** if it would fire more than five times in one Act (which is what happens on the Income-tax Act, left untouched rather than corrupted). It rewrote **three numbers in the whole library**, each checked by hand, and not one character of law text was lost. Schedules are now cited by name. And Ask now recognises a legal question from how it is asked — who the rule binds, whether it names a jurisdiction, whether it asks for a rule — and **never** when you are asking about your own paper. **Measured, no AI calls:** it now recognises **24 of 27** legal questions instead of 9, and — the number that matters — it has sent **zero** contract questions to the law library, out of 64 tried, including deliberately tricky ones like "Is the indemnity in our MSA legally binding under Indian law?". The right passage now reaches the answer for **every one of the 20** law questions, and **every citation is correct**. The safety check that was throwing questions away now blocks **none** — and its threshold was not touched. **One thing is blocking us, and it is not code: the AI account is out of credit** (the provider returns "payment required"). Six questions are newly reaching the right law but have never been answered, so I cannot yet tell you the answers are right — and I am not going to claim it. Everything is frozen exactly where it is until you top up; then it is one 23-question run. **Nothing is live** — this also needs the statute library rebuilt on the server when you are ready. |
