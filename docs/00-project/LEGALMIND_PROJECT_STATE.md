...
| **Health** | 1,110 backend + 184 frontend + 71 browser checks passing, none failing (1 skipped); CI (15 jobs) green on every push. *(Counts re-measured 2 Sep evening by running both suites — not carried over. Note: the visual baselines are expected to fail their next CI run once, deliberately — the workspace redesign changed the screenshots; the job's fresh captures get adopted per the standing rule.)* |
