...
| | |
|---|---|
| 🔧 **21 Sep — the law library was quietly broken, and it is fixed (NOT deployed; needs a re-index of the statutes)** | **What was wrong.** Indian statute PDFs print a tiny raised footnote number just before a section number, and it came out glued to it: the IT Act's section 66A read as "section 266A". Because the software assumes section numbers only go up, one glued digit made it swallow **every section after it** — the whole of the IT Act from 66A to 87 became a single unnamed lump, so **section 79, the rule on when a hosting company stops being protected from what its users post, had no address at all**. The same thing happened to the Civil Procedure Code (349 sections in one lump). Six such lumps held **57% of all the law text**, and because a lump matches almost any question, **31% of everything the search returned was one of them**. Separately, a **Schedule** (a table at the back of an Act) has no section number, so it was filed under whichever section came last — the DPDP Act's entire penalty table was being cited as "section 44, Amendments to certain Acts", which is simply the wrong reference. **What changed.** The glued digit is now stripped, but only when the Act's own contents page proves the number is impossible, and the repair **switches itself off** if it would fire more than five times in one Act — which is what happens on the Income-tax Act, whose publisher lays pages out differently; that Act is left exactly as it was rather than corrupted in a new way. It rewrote **three numbers in the entire library**, each checked by hand. Schedules are now their own citable unit. Not one character of law text was lost — verified across all 21 files. **Also fixed in the search:** an Act whose *title* happened to share a word with your question used to take every slot (asking about "reasonable security safeguards" returned ten hits from one set of Rules and buried the Act that actually answers), and a **repealed** Act used to win every tie against the Act in force. **Measured, no AI calls:** the right passage now reaches the answer for **16 of 20** law questions instead of 14, the right one is in the top three for 13 instead of 10, and **every citation is now correct (16 of 16, was 13 of 14)**. Speed unchanged. **Then tested for real, with the AI:** answers are short, plain and correctly sourced, and nothing from your company standards leaked into a general-law answer. **Not live.** Putting it live needs the statute library rebuilt on the server — and worth knowing: **the live library is older than we thought**, still carrying marks from before last week's reading-order fix, so it has never had that improvement either. **One thing still needs your decision.** For a law question that does not obviously look like one — "when does a hosting company lose its protection?" names no Act and no section — LegalMind requires the AI's own similarity score to clear a fixed bar before it will answer from the law library at all. The search now finds the right section for those questions, but that bar still throws it away, so **8 of 20 law questions are refused even though the answer was in hand**. That bar is there for a reason (it stops a contract question being answered from an unrelated Act) and you told me not to move it, so I have not. The options and what each costs are in the engineering record. |
