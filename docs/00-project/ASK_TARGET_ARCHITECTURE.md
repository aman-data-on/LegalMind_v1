...
| 6 Reranker | not built | **NOT BUILT** — already authorized by `AM-25` (hybrid retrieval *with reranking*) and `AM-26` (local, self-hosted, open-weight cross-encoder). Three separate measurements point at this as the next real gain. Needs only an `AM-26` r4 pin record, not a new authorization |
...
| **3 — feel** (~3 days) | SSE progress states (server yields deterministic stage strings; final event = verified answer). Prose markers → links to the citation list. Suggested follow-ups derived deterministically from `plan.topic` (sibling standards / sections). Answer prose formatting for point lists. | None | Blind pairwise preference (§7.6) ≥ 70% for the new pipeline; no internal term rendered (existing test) |
