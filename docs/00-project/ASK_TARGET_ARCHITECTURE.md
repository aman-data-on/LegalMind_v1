...
| **Generation** | `grounded-answer-2`, one blocking call; positions extractive only (AM-67 approved but **off**) | `grounded-answer-3`: answer-shape steering from the plan (lead with the answer, 1–4 cited points). **AM-67 enabled** — the reading aid beside the verbatim quote. Same `generate_raw` seam, same payload categories. |
...
- Domain A reading aid beside the verbatim quote (`AM-67`, approved, enable)
...
| **3 — feel** (~3 days) | **Prose markers → references: DONE 2026-09-17** (`AnswerProse`, both namespaces — document sources and statute sources, each scoped by turn id; the statute answer also gained the paragraph handling every other answer had). **SSE progress states: NOT DONE** — it needs `service.ask()` to report stage boundaries, which is the one file both PR #69 and PR #74 rewrite, and a new auth-bearing endpoint cannot be verified while `LEGALMIND_TEST_DATABASE_URL` is unprovisioned. **Suggested follow-ups: NOT DONE** — their specified source is `plan.topic`, and the planner ships off, so deriving them from anything else would be inventing a design (rule 4). | None | Blind pairwise preference (§7.6) ≥ 70% for the new pipeline; no internal term rendered (existing test) |
